...
# The create pass uses GZDoom's real -warp command-line autostart switch. With
# the Doom II-style Freedoom IWAD, `-warp 1` resolves to MAP01 before the main
# loop. The startup -exec file then leaves its delayed mutation/save sequence
# queued until the live level starts ticking. The load pass uses -loadgame.
...
Invoke-GZDoomScenario -Label "create-save" -ConfigPath $CreateCfg -ExtraArguments @("-warp", "1")
