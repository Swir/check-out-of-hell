...
### Changed
- Night Manager management-response reinforcements are widened from the original `12s / 26s / 42s / 58s` cadence to `15s / 34s / 54s / 76s`, preserving escalation while giving each threat more readable combat space.
- Ambient Overtime spawners and unfinished supervisor-wave spawners stop creating fresh enemies after the supervisor dies; enemies already alive stay active, so the clock-out leg remains an escape instead of an endlessly repopulating route.
- README/ROADMAP progress raised to 66% and Vertical Slice progress to 78% for the implemented/testable pacing milestone; the first genuinely fully polished Closing Time playtest remains the primary demo-readiness blocker.
